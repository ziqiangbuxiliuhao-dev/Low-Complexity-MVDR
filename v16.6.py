...
# ============================================================
# Correct Toeplitz-Persymmetric SSBF Helpers (新增/覆盖)
# ============================================================
def toeplitz_rectify(R):
    """正确的 Hermitian-Toeplitz 投影"""
    N = R.shape[0]
    rcol = np.zeros(N, dtype=np.complex128)
    for k in range(N):
        if k == 0:
            diag_vals = np.diag(R, 0)
        else:
            diag_vals_pos = np.diag(R, k)
            diag_vals_neg = np.diag(R, -k)
            diag_vals = np.concatenate([diag_vals_pos, np.conj(diag_vals_neg)])
        rcol[k] = np.mean(diag_vals)
    rcol[0] = np.real(rcol[0])
    return sla.toeplitz(rcol, np.conj(rcol))


def generate_Q_matrix(M):
    """Persymmetric 变换矩阵 Q（M 必须为偶数，框架中 N=32 满足）"""
    K = M // 2
...
# ============================================================
# Corrected Toeplitz-SSBF [26] —— Persymmetric + Toeplitz 子空间方法
# ============================================================
# ============================================================
# 改进版：支持非对称干扰的 Persymmetric-Toeplitz SSBF
# ============================================================
def beamformer_toeplitz_ssbf(
    Rhat,
    theta0_deg=0.0,
    main_half_width_deg=4.0,   # 忽略
    M_each=12,                 # 忽略
    Q=17,                      # 忽略
    L=3,                       # 忽略
    reg=1e-6,
    ...
    N = Rhat.shape[0]
    eta_0 = 1 + num_interferences

    # ==================== 关键修复1：Forward-Backward 平均 ====================
    J = np.fliplr(np.eye(N, dtype=np.complex128))
    R_fb = (Rhat + J @ Rhat.conj() @ J) / 2.0          # 强制统计 persymmetric

    # ==================== 关键修复2：Toeplitz 整流 ====================
    R_bar = toeplitz_rectify(R_fb)

    # ==================== persymmetric 实域变换（论文式16-17） ====================
    Q_mat = generate_Q_matrix(N)
    R_prime = np.real(Q_mat.conj().T @ R_bar @ Q_mat)

    # ==================== 实域特征分解 + SI 子空间（论文式18-20） ====================
    eigenvalues, eigenvectors = sla.eigh(R_prime)
    idx = np.argsort(eigenvalues)[::-1]
    E_s_prime = eigenvectors[:, idx[:eta_0]]
    Lambda_s_prime = np.diag(eigenvalues[idx[:eta_0]])

    # ==================== 目标导向矢量 ====================
    a0 = steering_deg(theta0_deg, N)
    alpha_0 = np.real(Q_mat.conj().T @ a0)

    # ==================== 实域权重计算（论文式21-23） ====================
    inv_Lambda = np.linalg.inv(Lambda_s_prime)
    b_0 = E_s_prime @ inv_Lambda @ E_s_prime.conj().T @ alpha_0

    scalar = np.sqrt(2) * np.real(b_0.conj().T @ alpha_0)
    w = (Q_mat @ b_0) / scalar
    w = w.reshape(-1, 1)
...
def run_methods_same_nodes(
    ...
    w_toep, _ = beamformer_toeplitz_ssbf(
        Rhat,
        theta0_deg=theta_s,
        num_interferences=len(theta_j)  # 保持原样
    )
...
def sweep_snr_plot(
    ...
    ax.plot(snr_list, sinr_prop_list,
            ...
            color=COLORS["proposed"], linestyle='-', linewidth=2.25, label='Proposed PCC-BF')
...
def main():
    ...
    print("Proposed method    : Proposed PCC-BF")
    ...
    print(f"Proposed PCC-BF    : {res['t_prop']:.6f} s")
    ...
    print(f"Proposed PCC-BF    : {db(res['sinr_prop']):.3f} dB")
